...
import { Search, Play, X, CheckCircle, Clock, User, GraduationCap, Award, BookOpen } from 'lucide-react';
...
export default function Courses() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      (data ?? []).forEach((p: CourseProgress) => { map[p.course_id] = p; });
      setProgress(map);
    })();
  }, [user]);
  ...
  const filtered = useMemo(() => {
    return courses.filter((c) => {
      ...
      const matchesSearch = !search || c.title.toLowerCase().includes(q) || (c.instructor?.toLowerCase().includes(q) ?? false);
  ...
  const markComplete = async (course: Course) => {
    ...
    if (existing) {
      await supabase.from('course_progress').update({ progress_percentage: 100, is_completed: true }).eq('id', existing.id);
    } else {
      await supabase.from('course_progress').insert({ user_id: user.id, course_id: course.id, progress_percentage: 100, is_completed: true });
    }
    setProgress((prev) => ({
      ...prev,
      [course.id]: { id: existing?.id ?? '', user_id: user.id, course_id: course.id, progress_percentage: 100, is_completed: true, updated_at: new Date().toISOString() },
    }));
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          {user && completedCount > 0 && (
            <div className="mt-6 inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-gold-500/20 border border-gold-500/30">
              ...
              <span className="text-white font-medium">{completedCount} course{completedCount !== 1 ? 's' : ''} completed</span>
            </div>
          )}
        </div>
      </section>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {filtered.map((course, i) => {
                ...
                return (
                  ...
                  >
                    <button onClick={() => setSelectedCourse(course)} className="relative w-full h-48 overflow-hidden bg-gradient-to-br from-primary-100 to-gold-100 dark:from-slate-800 dark:to-slate-700">
                    ...
                    <div className="p-5">
                      <span className="text-xs font-semibold px-2.5 py-1 rounded-lg bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300 mb-2 inline-block">{course.category}</span>
                      ...
                      <p className="text-sm text-slate-500 dark:text-slate-400 mb-3 flex items-center gap-1">
                        <User className="h-3.5 w-3.5" /> {course.instructor ?? 'Unknown'}
                      </p>
                      {prog && (
                        <div className="mb-3">
                          <div className="flex items-center justify-between text-xs mb-1">
                            <span className="text-slate-500 dark:text-slate-400">Progress</span>
                            <span className="font-semibold">{prog.progress_percentage}%</span>
                          </div>
                          <div className="h-1.5 rounded-full bg-slate-200 dark:bg-slate-700 overflow-hidden">
                            <div className="h-full bg-gradient-to-r from-primary-500 to-gold-500 rounded-full transition-all duration-500" style={{ width: `${prog.progress_percentage}%` }} />
                          </div>
                        </div>
                      )}
                      <button onClick={() => setSelectedCourse(course)} className="w-full py-2.5 rounded-xl bg-gradient-to-r from-primary-600 to-primary-700 text-white text-sm font-semibold hover:scale-[1.02] transition-transform">
          ...
          >
            ...
            >
              <div className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-700">
                <div>
                  <span className="text-xs font-semibold px-2.5 py-1 rounded-lg bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300">{selectedCourse.category}</span>
                  <h2 className="text-xl font-bold mt-1">{selectedCourse.title}</h2>
                </div>
                <button onClick={() => setSelectedCourse(null)} className="p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-700">
                  <X className="h-5 w-5" />
                </button>
              </div>
              ...
              <div className="p-6">
                <div className="flex flex-wrap items-center gap-4 text-sm text-slate-500 dark:text-slate-400 mb-4">
                  <span className="flex items-center gap-1"><Clock className="h-4 w-4" /> {selectedCourse.duration ?? 'N/A'}</span>
                  <span className="flex items-center gap-1"><User className="h-4 w-4" /> {selectedCourse.instructor ?? 'Unknown'}</span>
                </div>
                <p className="text-slate-700 dark:text-slate-300 leading-relaxed mb-6">{selectedCourse.description}</p>
