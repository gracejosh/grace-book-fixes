...
import type { BibleVerse, Book, Course, Quiz as QuizType, Profile, PrayerRequest, ContactMessage, NewsletterSubscriber, PopupAd } from '@/types';
import { Lock, LayoutDashboard, BookOpen, Library, GraduationCap, BrainCircuit, Users, Mail, Plus, Edit2, Trash2, X, Download, TrendingUp, Award, MessageSquare, Heart, FileText, Megaphone, Ban, ShieldCheck } from 'lucide-react';
import { EmptyState } from '@/components/ui';

const ADMIN_PASSWORD = 'grace2024';
type Tab = 'dashboard' | 'verses' | 'books' | 'courses' | 'quizzes' | 'users' | 'messages' | 'ads';
...
function BooksTab({ showToast }: { showToast: (m: string, t?: 'success' | 'error' | 'info' | 'warning') => void }) {
  ...
  const [form, setForm] = useState({ title: '', author: '', description: '', cloudinary_url: '', cover_url: '', category: 'General', file_format: 'PDF', price_type: 'free' as 'free' | 'paid', apple_books_url: '', epub_url: '' });
  ...
  const save = async () => {
    ...
    setForm({ title: '', author: '', description: '', cloudinary_url: '', cover_url: '', category: 'General', file_format: 'PDF', price_type: 'free', apple_books_url: '', epub_url: '' });
    load();
  };
  ...
  return (
    <div>
      <CrudHeader title="Books" onAdd={() => { setEditing(null); setForm({ title: '', author: '', description: '', cloudinary_url: '', cover_url: '', category: 'General', file_format: 'PDF', price_type: 'free', apple_books_url: '', epub_url: '' }); setShowForm(true); }} />
      {loading ? <div className="skeleton h-64 rounded-xl" /> : (
          ...
          {items.map((b) => (
            <div key={b.id} className="glass-card p-4">
              <div className="flex items-start justify-between gap-2 mb-2">
                ...
                <div className="flex gap-1">
                  <button onClick={() => { setEditing(b); setForm({ title: b.title, author: b.author, description: b.description ?? '', cloudinary_url: b.cloudinary_url, cover_url: b.cover_url ?? '', category: b.category, file_format: b.file_format, price_type: b.price_type ?? 'free', apple_books_url: b.apple_books_url ?? '', epub_url: b.epub_url ?? '' }); setShowForm(true); }} className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700"><Edit2 className="h-3.5 w-3.5 text-slate-400" /></button>
              ...
              <p className="text-xs text-slate-500">by {b.author} · {b.category} · {b.file_format} · <span className={b.price_type === 'free' ? 'text-emerald-500 font-semibold' : 'text-gold-500 font-semibold'}>{b.price_type ?? 'free'}</span></p>
            </div>
          ))}
        </div>
      )}
      <FormModal show={showForm} onClose={() => setShowForm(false)} title={editing ? 'Edit Book' : 'Add Book'} onSave={save}>
        ...
        <select value={form.price_type} onChange={(e) => setForm({ ...form, price_type: e.target.value as 'free' | 'paid' })} className="input-field">
          <option value="free">Free</option>
          <option value="paid">Paid</option>
        </select>
        {form.price_type === 'paid' && (
          <input value={form.apple_books_url} onChange={(e) => setForm({ ...form, apple_books_url: e.target.value })} placeholder="Apple Books URL" className="input-field" />
        )}
        <input value={form.epub_url} onChange={(e) => setForm({ ...form, epub_url: e.target.value })} placeholder="EPUB download URL (optional)" className="input-field" />
      ...
      <FormModal show={showForm} onClose={() => setShowForm(false)} title={editing ? 'Edit Question' : 'Add Question'} onSave={save}>
        ...
        {form.options.map((opt, i) => (
          <div key={i} className="flex items-center gap-2">
            <input type="radio" name="correct_answer" checked={form.correct_answer === i} onChange={() => setForm({ ...form, correct_answer: i })} className="w-4 h-4 accent-primary-600" />
            <span className="w-6 h-6 rounded-lg bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300 text-xs font-bold flex items-center justify-center shrink-0">{String.fromCharCode(65 + i)}</span>
            <input value={opt} onChange={(e) => { const next = [...form.options]; next[i] = e.target.value; setForm({ ...form, options: next }); }} placeholder={`Option ${String.fromCharCode(65 + i)}`} className="input-field" />
          </div>
        ))}
        <p className="text-xs text-slate-500">Select the radio button next to the correct answer (A/B/C/D).</p>
        <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className="input-field">
...
function UsersTab({ showToast }: { showToast: (m: string, t?: 'success' | 'error' | 'info' | 'warning') => void }) {
  ...
  const toggleBan = async (p: Profile) => {
    ...
    showToast(`${p.username} ${p.is_banned ? 'unbanned' : 'banned'}`, 'info');
    load();
  };

  const deleteUser = async (p: Profile) => {
    if (!confirm(`Delete user ${p.username}? This will remove their profile.`)) return;
        ...
        <div className="space-y-2">
          {users.map((u) => (
            <div key={u.id} className="glass-card p-4 flex items-center gap-4">
              ...
              <button onClick={() => toggleBan(u)} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${u.is_banned ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300' : 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 hover:scale-105'}`}>
                {u.is_banned ? <span className="flex items-center gap-1"><ShieldCheck className="h-3 w-3" /> Unban</span> : <span className="flex items-center gap-1"><Ban className="h-3 w-3" /> Ban</span>}
              </button>
              ...
              <button onClick={() => deleteUser(u)} className="p-1.5 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/30 transition-all"><Trash2 className="h-4 w-4 text-red-400" /></button>
            </div>
          ))}
        </div>
      )}
...
function AdsTab({ showToast }: { showToast: (m: string, t?: 'success' | 'error' | 'info' | 'warning') => void }) {
  ...
  const [form, setForm] = useState({ title: '', content: '', image_url: '', link_url: '', is_active: false });
  ...
  const save = async () => {
    if (editing) {
      await supabase.from('popup_ads').update(form).eq('id', editing.id);
    } else {
      await supabase.from('popup_ads').insert(form);
    }
    showToast(editing ? 'Ad updated' : 'Ad created', 'success');
    setShowForm(false); setEditing(null);
    setForm({ title: '', content: '', image_url: '', link_url: '', is_active: false });
    load();
  };
  ...
  const toggleActive = async (ad: PopupAd) => {
    await supabase.from('popup_ads').update({ is_active: !ad.is_active }).eq('id', ad.id);
    showToast(`Ad ${!ad.is_active ? 'activated' : 'deactivated'}`, 'info');
    load();
  };

  return (
    <div>
      <CrudHeader title="Popup Ads" onAdd={() => { setEditing(null); setForm({ title: '', content: '', image_url: '', link_url: '', is_active: false }); setShowForm(true); }} />
      {loading ? <div className="skeleton h-64 rounded-xl" /> : ads.length === 0 ? (
        <div className="glass-card p-8 text-center">
          <Megaphone className="h-12 w-12 text-primary-300 mx-auto mb-3" />
          <p className="text-slate-500">No ads yet. Create one to show popup ads to visitors every 30 minutes.</p>
        </div>
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {ads.map((ad) => (
            <div key={ad.id} className="glass-card p-4">
              <div className="flex items-start justify-between gap-2 mb-2">
                ...
                <div className="flex gap-1">
                  <button onClick={() => { setEditing(ad); setForm({ title: ad.title, content: ad.content ?? '', image_url: ad.image_url ?? '', link_url: ad.link_url ?? '', is_active: ad.is_active }); setShowForm(true); }} className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700"><Edit2 className="h-3.5 w-3.5 text-slate-400" /></button>
              ...
              {ad.image_url && <img src={ad.image_url} alt={ad.title} className="w-full h-24 object-cover rounded-lg mb-2" />}
              <p className="text-xs text-slate-500 mb-2 line-clamp-2">{ad.content}</p>
              <button onClick={() => toggleActive(ad)} className={`w-full px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${ad.is_active ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300' : 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300'}`}>
      ...
      <FormModal show={showForm} onClose={() => setShowForm(false)} title={editing ? 'Edit Ad' : 'Add Ad'} onSave={save}>
        <input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="Ad title" className="input-field" />
        <textarea value={form.content} onChange={(e) => setForm({ ...form, content: e.target.value })} placeholder="Ad content" className="input-field min-h-[60px]" />
        <input value={form.image_url} onChange={(e) => setForm({ ...form, image_url: e.target.value })} placeholder="Image URL" className="input-field" />
        <input value={form.link_url} onChange={(e) => setForm({ ...form, link_url: e.target.value })} placeholder="Link URL (optional)" className="input-field" />
        <label className="flex items-center gap-2 text-sm font-medium">
          <input type="checkbox" checked={form.is_active} onChange={(e) => setForm({ ...form, is_active: e.target.checked })} className="w-4 h-4 accent-primary-600" />
