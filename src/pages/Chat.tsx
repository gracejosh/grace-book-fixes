                ...
                <div className="space-y-1">
                  {filteredUsers.map((person) => (
                    ...
                    >
                      ...
                      <span className="text-xs text-primary-600 dark:text-primary-400"><MessageCircle className="h-3.5 w-3.5" /> Message</span>
                    </motion.button>
                  ))}
                </div>
              </AnimatePresence>
              ...
              <div className="p-6 text-center">
                ...
                <div className="mt-5 flex flex-col gap-2 sm:flex-row sm:justify-center">
                  <button
                    ...
                    className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl bg-primary-600 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-primary-700 disabled:cursor-wait disabled:opacity-70"
                  >
                    {openingUserId === selectedUser.id ? <Loader className="h-4 w-4 animate-spin" /> : <MessageCircle className="h-4 w-4" />}
                    {openingUserId === selectedUser.id ? 'Opening chat...' : 'Message'}
                  </button>
                  <button
                    type="button"
                    onClick={() => openUserProfile(selectedUser.id)}
                    className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl border border-slate-200 px-5 py-2.5 text-sm font-semibold text-slate-600 transition hover:bg-slate-100 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"
                  >
                    View profile
                  </button>
                </div>
